...
# for single output in single row
    ...
    # Add this line to the top of the model loop
    model_replies = {}
    
    # === Inside your existing model loop ===
    for model_name in selected_models:
        ...
        with st.spinner(f"Thinking with {model_name}..."):
            ...
            # Use model-specific session history key
            model_session_key = f"{session_id}_{model_name}"
            if model_session_key not in st.session_state.store:
                st.session_state.store[model_session_key] = ChatMessageHistory()
            session_history = st.session_state.store[model_session_key]
            ...
            if uploaded_files and retriever is not None:
                ...
                def get_session_history(_: str) -> BaseChatMessageHistory:
                    return st.session_state.store[model_session_key]
                ...
                response = conversational_rag_chain.invoke(
                    ...
                    config={"configurable": {"session_id": model_session_key}},
                )
            ...
            # Temporarily store model response
            model_replies[model_name] = assistant_reply
    ...
    # === After the model loop: Save to global session history once ===
    if model_replies:
        global_session_history = st.session_state.store.get(session_id, ChatMessageHistory())
        global_session_history.add_user_message(user_input)
        for model_name, reply in model_replies.items():
            global_session_history.add_ai_message(f"{model_name}: {reply}")
