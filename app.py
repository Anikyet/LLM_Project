...
# for single output in single row
    Selected_model=""
    original_history = list(session_history.messages)  # Clone initial state of chat history
    ...
    for model_name in selected_models:
        ...
        with st.spinner(f"Thinking with {model_name}..."):
            ...
            # Create a temporary history object for isolation
            temp_history = ChatMessageHistory()
            temp_history.messages = list(original_history)  # Set same messages for all models
    
            if uploaded_files and retriever is not None:
                def get_session_history(_: str) -> BaseChatMessageHistory:
                    return temp_history  # isolated per model
                ...
                messages = qa_prompt.format_messages(
                    input=user_input,
                    chat_history=temp_history.messages,
            ...
            st.markdown(
                f"""<div style="border-radius: 10px; padding: 1rem; font-size: 16px; line-height: 1.6; color: white; font-weight: 600;">{assistant_reply}</div>""",
            ...
            # Optional: Add evaluation result display here
    
        st.divider()
        st.markdown("<div style='margin-top: 50px;'></div>", unsafe_allow_html=True)
    
    # ✅ Only add user + assistant once (e.g., for the first model or general)
    session_history.add_user_message(user_input)
    session_history.add_ai_message(f"{selected_models[0]}: {assistant_reply}")  # or just assistant_reply if not model-specific
    st.rerun()
